...
public class RenderUtils {
	...
	static {
		...
		// Scale the points down (as JaamSim uses a 1x1 box [-0.5, 0.5] not [-1, 1]   每个元素乘0.5缩小
	...
	public static List<Vec4d> transformPoints(Mat4d mat, List<Vec4d> points, int dummy) {
		...
		for (Vec4d p : points) {
			...
			//矩阵乘集合中的向量p，并将结果赋给向量v，然后将向量v添加到集合中
			v.mult4(mat, p);
			ret.add(v);
		}
		return ret;
	...
	public static List<Vec3d> transformPointsWithTrans(Mat4d mat, List<Vec3d> points) {
		...
		for (Vec3d p : points) {
			Vec3d v = new Vec3d();
			//三维向量v调用矩阵乘向量的方法，并将结果赋给三维向量v，同时将v添加到集合中
...
static void putPointXYZW(FloatBuffer fb, Vec4d v) {
	 ...
	 * 返回多边形或星形的点列表。
	 * @param radius - distance from the centre of the polygon or star to each vertex  多边形的中心到每个顶点的距离
	 * @param n - number of vertices   顶点的数量
	 * @param m - number of vertices spanned by each line   每条线张成的顶点数
	 * @return list of coordinates for the vertices  顶点的坐标列表
	 */
	 ...
	 * 建立一个圆角矩形(类似于现有的库存)。假设舍入width-wise
	 * @param width
	 * @param height
	 * @return
	 */
	public static ArrayList<Vec4d> getRoundedRectPoints(double width, double height, int numSegments) {
		...
		// Create semi circles on the ends 在两端画半圆
		double xScale = 1;
	 ...
	 * 返回一些可以绘制圆弧的点。这将返回行对(与getCirclePoints()不同，后者返回行带的点)。
	 * @param radius 半径
	 * @param center 中心点
	 * @param startAngle 开始角度
	 * @param endAngle	结束角度
	 * @param numSegments 点的个数
	 * @return
	 */
