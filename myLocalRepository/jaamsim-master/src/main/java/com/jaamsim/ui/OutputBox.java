...
public class OutputBox extends FrameBox {
	...
	/**
	 * Returns the only instance of the property box返回属性框的唯一实例
	 */
	...
	public void setEntity( Entity entity ) {
		...
		// Build up the row list, leaving extra rows for entity names构建行列表，为实体名称保留额外的行
		...
		for (OutputHandle h : handles) {
			...
			if (currClass != klass) {
				// This is the first time we've seen this class, add a place holder row  添加一个占位符行
				currClass = klass;
				entries.add(klass);
			}
			entries.add(h);
