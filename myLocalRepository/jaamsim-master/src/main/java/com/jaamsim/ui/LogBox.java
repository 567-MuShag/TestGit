...
public class LogBox extends FrameBox {
	 ...
	 * 记录一个格式化的字符串，有效的包装String.format
	 * @param format
	 * @param args
	 */
	...
	public static void logLine(final String logLine) {
		synchronized(logLock) {
			//logBuilder是一个StringBuilder,将logLine添加到StringBuilder中然后换行
			logBuilder.append(logLine + "\n");
		}

		// Append to the existing log area if it exists，如果现有日志区域存在，则将其追加
		if (myInstance != null) {
			//SwingUtilities是Swing使用方法的集合
			//导致doRun.run()在AWT事件指派线程上异步执行，在所有挂起的AWT事件被处理后才发生，此方法应该在应用程序线程需要更新该GUI时使用。
			SwingUtilities.invokeLater(new Runnable() {
				@Override
				public void run() {
					//logArea是JTextArea一个显示纯文本的多行区域
