...
/**
 * Class to display information about model objects. <br>类来显示有关模型对象的信息
 * displays the results of getInfo for an object   显示对象的getInfo结果
 */
public class EditBox extends FrameBox {
	...
	private EditBox() {
		...
		// Provide tabs for the editor该组件允许用户通过单击具有给定标题图标的选项卡，在一组组件之间进行切换
		...
		// Set the size and position of the editor设置编辑器的大小和位置
		...
		// Save changes to the editor's size and position保存对编辑器的大小和位置的更改
	...
	public void setEntity(Entity entity) {
		if (entity == currentEntity) {
			// Reset the title in case the entity's name has changed如果实体的名称发生更改，则重置标题
...
private static class CategoryInputs {
	...
	private static ArrayList<CategoryInputs> getInputs(Entity ent) {
		// create a list of Inputs sorted by category创建一个按类别排序的输入列表
		...
		// assuming that editable inputs of the same category are adjacent假设同一类别的可编辑输入是相邻的
		for (Input<?> in : sortedInputs) {
			// the first time entering the loop 第一次进入循环
			if (cat.isEmpty())
				cat = in.getCategory();

			// new category (the previous category inputs is done so add it to the list)新类别(以前的类别输入已完成，因此将其添加到列表中
...
public static class EditTable extends JTable {
	...
	public TableCellEditor getCellEditor(int row, int col) {
		...
		// If the user is going to retry a failed edit, update the editor with the old value如果用户要重试失败的编辑，请使用旧值更新编辑器
	...
	public String getToolTipText(MouseEvent e) {
		...
		// not necessary because reordering of tabs is not allowed没有必要，因为不允许重新排序制表符
		colIndex = convertColumnIndexToModel(colIndex);

		// Only the keyword column has tooltip只有关键字列有工具提示
		if (colIndex != 0)
			return null;
