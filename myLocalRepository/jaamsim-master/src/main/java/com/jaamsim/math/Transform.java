 ...
 * 变换类表示由旋转、缩放和平移组成的三维线性变换。在内部，它将所有3个转换存储为离散转换，但允许将任意转换链接、折叠和反转。主要的限制是它只允许均匀(各向同性)缩放
 ...
 * 来自这个新变换的矩阵将与两个变换的矩阵相乘时的矩阵相同(参见单元测试)
 ...
 * 返回与此转换相反的转换，以任意顺序合并这两个转换将导致标识转换
 * @param out
 */
