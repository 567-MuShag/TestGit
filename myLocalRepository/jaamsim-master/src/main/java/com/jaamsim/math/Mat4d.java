 ...
 * 根据传入的大于等于16个参数，选取前16个参数，按照参数的顺序构造一个4阶矩阵
 * 该方法的参数是可变参数
 * @throws NullPointerException if mat is null
...
public void transpose4(Mat4d m) {
	// We can safely set the diagonal, even if m is 'this'   将这个矩阵的对角线设置为给定矩阵的对角线
 ...
 * 计算传入的4阶矩阵m1和m2的乘法，最终结果只取3X3矩阵，并将这个3X3矩阵赋给这个矩阵
 ...
 * 将这个矩阵乘上传入的m矩阵，最后 取3X3矩阵，并将该矩阵赋给这个矩阵
 * @throws NullPointerException if m is null
 */
...
public double determinant() {
	...
	//当最后一行趋向于0,0,0,1时，计算沿着这一行的系数展开，其中快径为零。
	double det = 0.0d;
	if (d30 != 0.0d) {
...
private double cofactor(int x, int y, double[] data, double[] sub) {
	...
	// Now determine the determinant of the submat   现在确定子mat的行列式
	double ret = 0;
