 ...
 * 创建一个由法线定义的平面，并且距离原点最近，这是一种存储格式，类似于平面常用的数学定义
 * @param norm
 * @param distance
 */
 ...
 * 通过坐标变换变换平面p并将结果存储在'this'中
 * @param t - the Transform  变换
 * @param p - the plane to transform  要变换的平面
 */
 ...
 * 用坐标变换矩阵“mat”变换平面p，并将结果存储在“this”中
 * @param mat - the Transform matrix   变换矩阵
 * @param normalMat - the normal matrix, should be the inverse transpose of 'mat' or equal to 'mat' if there is no non-uniform scaling  正规矩阵，应该是mat的逆转置或者等于mat如果没有不均匀缩放的话
 * @param p - the plane to transform   要变换的平面
 */
public void transform(Mat4d mat, Mat4d normalMat, Plane p) {
	...
	// The point closest to the origin (need any point on the plane  最接近原点的点(需要平面上的任意一点)
	closePoint.scale3(p._dist, p.normal);
	// Now close point is the transformed point   闭点是经过变换的poin
 ...
 * 得到一条射线与这个平面碰撞的距离，这个可以返回如果光线是平行的，就是无穷大
 * @param r
 * @return
 */
public double collisionDist(Ray r) {

	// cos = plane-Normal dot ray-direction  plane-Normal点射线方向
	double cos = -1 * normal.dot3(r.getDirRef());

	if (MathUtils.near(cos, 0.0)) {
		// The ray is nearly parallel to the plane, so no collision光线几乎平行于平面，所以没有碰撞
		//POSITIVE_INFINITY保存 double 类型的正无穷大值的常量
...
//返回两个平面碰撞产生的“光线”，如果两个平面平行则返回null
public Ray collide(Plane p) {
	...
	// Ray dir is the direction of the new ray  射线dir是新射线的方向
	...
	//现在，我们需要在两个平面上都有一个点，所以我们会找到这个平面上的任意一条射线并与另一条射线相交
	Vec3d intDir = new Vec3d();
	intDir.cross3(rayDir, normal); // Take the cross of our new direction and the normal, this must be in the plane求新方向和法向量的叉乘，它一定在平面上
	intDir.normalize3();
	Vec3d intStart = new Vec3d(normal);
	intStart.scale3(_dist); // intStart is in this plane   intStart在这个平面上
