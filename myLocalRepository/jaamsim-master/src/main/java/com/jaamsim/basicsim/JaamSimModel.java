...
public class JaamSimModel {
	...
	public final Entity getNamedEntity(String name) {
		synchronized (allInstances) {
			//namedEntities是一个HashMap集合，根据传入的键获取响应的值
	 ...
	 * 当recorde为真时，任何被改变的模型输入和任何被改变的对象，被定义为“已编辑”。当为假时，模型输入和对象，定义被标记为“未编辑”
	 * <p>
	 ...
	 *recorde模式用于确定JaamSim保存配置文件的方式，通过图形用户界面。对象定义和模型输入，被标记为未编辑的内容将被完全复制，就像它们在原文中出现一样，首次加载的配置文件。对象定义和模型输入，标记为已编辑的内容将由程序自动生成。
	 * @return the RecordEdits mode for the InputAgent.
	 */
	public boolean isRecordEdits() {
		//返回是否是isRecordEdits模式
		return recordEdits;
	}
